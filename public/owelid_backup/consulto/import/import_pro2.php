<?php 
require_once($_SERVER['DOCUMENT_ROOT'].'/bmscl/db_conn.php');
	...
	else
	{
		if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
			...
			if (($getdata = fopen($target_file, "r")) !== FALSE) {
			 ...
			 $data_c_key = mysqli_real_escape_string($conn ,$columnData[0]);
			 $data_c_value = mysqli_real_escape_string($conn ,$columnData[1]);
			 $data_c_id = mysqli_real_escape_string($conn ,$columnData[2]);
			 
			 $import_data[]="('".$data_c_key."','".$data_c_value."','".$data_c_id."')";
			// SQL Query to insert data into DataBase
 
			 }
			 $import_data = implode(",", $import_data);
			 $query = "INSERT INTO course_info(c_key,c_value,c_id) VALUES  $import_data ;";
