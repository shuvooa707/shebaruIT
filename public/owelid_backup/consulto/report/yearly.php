...
<script defer src="http://www.fusioncharts.com/dev/assets/fusioncharts/themes/fusioncharts.theme.zune.js"></script>

<div id="page-wrapper">
    <div class="row">
        <div class="col-lg-12">
            <h3 class="page-header">Yearly Report</h3>
		</div>
        <!-- /.col-lg-12 -->
    </div>
    <!-- /.row -->	
...
$result_count_user = mysqli_num_rows($result_query_user);
  ...
  // If the query returns a valid response, prepare the JSON string
    ...
    $arrData = array(
      "chart" => array(
                  "caption"=>"Student File Report",
                  "subCaption"=>"Number of submitted files of this year",
                  "theme"=>"ocean"
        )
    );

	$arrData["data"] = array();
	$range1 = date('Y-m-d', strtotime('first day of January'));
	$range2 = date('Y-m-d', strtotime('today'));
	$total = 0;
	...
	$count_student = "SELECT COUNT(s.id) AS nos FROM student s
				...
				WHERE (assessment.counselor_id=".$row_query_user['id']. " OR assessment.appointed_to=".$row_query_user['id'].") AND s.date BETWEEN '".$range1."' AND '".$range2."'";
  ...
  // If the query returns a valid response, prepare the JSON string
    ...
    $arrData1 = array(
      "chart" => array(
                  ...
                  "subCaption"=>"Number of assessment of this year",
                  "theme"=>"zune"
        )
    );
	...
	$count_student1 = "SELECT COUNT(id) AS nos FROM assessment
				WHERE (counselor_id=".$row_query_user1['id']. " OR appointed_to=".$row_query_user1['id'].") AND date BETWEEN '".$range1."' AND '".$range2."'";
...
$(document).ready(function(){
    //$(".raphael-group-104-creditgroup").hide();
