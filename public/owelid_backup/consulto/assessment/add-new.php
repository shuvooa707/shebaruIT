...
if (isset($_POST['submit'])){
	...
	{
		$certificate = mysqli_real_escape_string($_REQUEST['certificate']);
	...
	{
		$year = mysqli_real_escape_string($_REQUEST['year']);
	...
	{
		$cgpa = mysqli_real_escape_string($_REQUEST['cgpa']);
	...
	{
		$group =mysqli_real_escape_string($_REQUEST['group']);
	...
	{
		$board = mysqli_real_escape_string($_REQUEST['board']);
	...
	if(mysqli_query($conn,$query_assessment))
	{
		if($counselor != 0)
		{
			...
			$query_notification = "INSERT INTO notification(message, link, counselor_id) VALUES ('New Assessment','assessment/view-assessment.php?id=$last_id', $counselor)";
			mysqli_query($conn,$query_notification) or die(mysqli_error($conn));
		}
...
<div id="page-wrapper">
	...
	if(isset($_GET['success']))
		echo "<div class='alert alert-success'>Success! <a href='view-assessment.php?id=".mysqli_insert_id($conn)."'>View Assessment</a></div>";
	if(isset($_GET['error']))
	{
...
<?php
$max_fn = mysqli_fetch_assoc(mysqli_query($conn, "SELECT MAX(form_no) FROM assessment"));
...
<form id="signupform" role="form" method="post" action="add-new.php"> 
	...
	<div class="row">
		<div class="col-lg-3">
			<div class="form-group">
				...
				<select class="form-control" name="office" data-validation="required">
					...
					<option value="Uttara" <?php if($row_user['office']=='Uttara') echo "selected"; ?>>Uttara Office</option>
		...
		<div class="col-lg-3">
			<div class="form-group">
				...
				<input class="form-control" type="text" name="formno" placeholder="XXXX" value="<?php echo $max_fn['MAX(form_no)']+1 ?>" data-validation="required">
			</div>
		</div>
		<div class="col-lg-3">
			<div class="form-group">
		...
		<div class="row">
			<div class="col-lg-4">
				<div class="form-group">
					...
					<input type="text" class="form-control" name="dob" placeholder="YYYY-MM-DD" value="">				
				</div>
			</div>
			<div class="col-lg-4">
				<div class="form-group">
...
$('input[name="dob"]').daterangepicker({
  ...
  $('input[name="dob"]').val(chosen_date.format('YYYY-MM-DD'));
});
</script>