...
<?php
$query_student = "SELECT * FROM student WHERE active=1 AND completed=0 ORDER BY gd_no DESC";
...
$query_student = "SELECT *, s.id, s.name, s.university, s.program, s.subject FROM student s
					...
					WHERE (assessment.counselor_id=".$row_user['id']. " OR assessment.appointed_to=".$row_user['id'].") AND s.active=1 AND s.completed=0";
$result_query_student = mysqli_query($conn, $query_student);
$result_count_student = mysqli_num_rows($result_query_student);
}
...
<?php } ?>
	<div class="tab-content" style="margin-top:20px">
			...
			<div class="row">
					...
					while($row_query_student = mysqli_fetch_assoc($result_query_student)) {
						$query_counselor = "SELECT user.name FROM user
											...
											WHERE student.id=".$row_query_student['id']." AND student.active=1 AND student.completed=0";
					...
					<div class="col-lg-4 col-sm-6 col-xs-12">
						<div class="well" style="background:#fff;">
							...
							<h4 class="text-primary"><a href="view-student.php?id=<?php echo $row_query_student['id']; ?>"><?php echo $row_query_student['name']; ?></a></h4>
							...
							<div class="col-lg-4-offset" style="width:130px;display: block; margin: 0 auto;">
								<a href="view-student.php?id=<?php echo $row_query_student['id']; ?>" class="btn btn-sm btn-default"><i class="fa fa-eye" aria-hidden="true"></i> View</a>
								<a href="edit-student.php?id=<?php echo $row_query_student['id']; ?>" class="btn btn-sm btn-default"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</a>
							</div>
						</div>
					</div>
		...
		<div id="<?php echo $row_query_user['id']; ?>" class="tab-pane fade">
			...
			$query_student = "SELECT *, s.id, s.name, s.university, s.program, s.subject FROM student s
								INNER JOIN assessment ON assessment.id=s.assessment_id
								WHERE (assessment.counselor_id=".$row_query_user['id']. " OR assessment.appointed_to=".$row_query_user['id'].") AND s.active=1 AND s.completed=0";
			$result_query_student = mysqli_query($conn, $query_student);
			$result_count_student = mysqli_num_rows($result_query_student);
			?>
			<div style='margin-bottom:20px; font-size:18px;'><div class='label label-success'>Total Available: <?php echo $result_count_student; ?></div></div>
